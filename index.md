---
title: Student Activity Guide
nav_order: 1
---

# Using GitHub & AI to Create & Host Websites — Student Activity Guide
...
7. Complete the **verification puzzle**.
   - **iPad tip:** If the puzzle pieces are hard to tap, try rotating your iPad to landscape mode — the pieces are bigger.
...
5. Go to your repo on **github.com** → click **`index.html`** → **pencil icon** → select all → delete → paste → commit.
   - **iPad tip:** If you don't see the pencil icon, tap the **"…"** (three dots) menu at the top right and look for "Edit".
   - **With a keyboard:** `Cmd+A` to select all, then `Cmd+V` to paste.
   - **Without a keyboard:** Tap and hold → "Select All" → delete → tap and hold → "Paste".
   - **iPad copy tip:** When copying code from the AI tool, look for a **clipboard icon** or **"Copy" button** on the code block — tap that instead of trying to select all the code by hand.
...
1. Open **GitHub Copilot** at [github.com/copilot](https://github.com/copilot).
   > **iPad Split View tip:** You can open two Safari windows side by side! Drag a tab to the left or right edge of the screen. Put your AI assistant on one side and GitHub on the other — much easier than switching tabs.
